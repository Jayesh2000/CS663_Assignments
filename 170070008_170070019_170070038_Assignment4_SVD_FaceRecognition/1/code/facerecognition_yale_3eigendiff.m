function [] = facerecognition_yale_3eigendiff()
...
%disp(size(U);
eigen_coeff_training = U(:,4:1: end-1)'*mean_centered_training_set;  % size k x 1520
...
for i = 1:1:39
    if(i < 14) 
        ...
        for j=41:1:size(y,2)-2
            ...
            img = imread(filename2);
            img = reshape(img,[],1);
            eigen_coeff_testing = U(:,4:1: end-1)'*(double(img(:)) - mean);  %10*1
...
ylabel('Recognition rate');
 title('SVD on Yale database with top 3 eigen-vectors removed');
end