function myBilinearInterpolation(M)
...
%Doing bilinear interpolation of the rows

for i=1:1:r2-1
    for j=1:2:c2
        if mod(i,3) == 2
            res(i,j)=(2*res(i-1,j)/3+res(i+2,j)/3);
        end
        if mod(i,3) == 0
            res(i,j)=(res(i-2,j)/3+2*res(i+1,j)/3);
        end        
    end
end
...
for i=1:1:r2
    for j=2:2:c2
        res(i,j)=(res(i,j-1)/2+res(i,j+1)/2);       
    end
end
J=mat2gray(res);
RI = imref2d(size(J));
...
title('Original');
suptitle('Bilinear Interpolation');
end
